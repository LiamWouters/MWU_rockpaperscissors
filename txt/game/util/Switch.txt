...
class Switch(UIelement):
    def __init__(self, 
        ...
        if size[0] == None or size[1] == None:
            largest_text_size = [max(self.opt1_rect.width, self.opt2_rect.width), max(self.opt1_rect.height, self.opt2_rect.height)]
            largest_text_size[0] = largest_text_size[0] + (2*textPaddingX)
            full_width = (2*largest_text_size[0]) + slider_size[0]
            full_height = (2*largest_text_size[1]) + slider_size[1]
    ...
    def _update_color(self):
        ...
        if self.slider_bg_rect.collidepoint(mousePos):
